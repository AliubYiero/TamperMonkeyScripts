...
class UiMenu {
	...
	get sendDelayInput(): HTMLInputElement {
		...
		inputElement.addEventListener( 'change', () => {
			...
			( <HTMLInputElement> this.domList.sendDelayMaxInput ).min = inputElement.value;
			if ( ( <HTMLInputElement> this.domList.sendDelayMaxInput ).value < inputElement.value ) {
				( <HTMLInputElement> this.domList.sendDelayMaxInput ).value = inputElement.value;
			}
		} )
	...
	get sendDelayMaxInput(): HTMLInputElement {
		...
		inputElement.min = ( <HTMLInputElement> this.domList.sendDelayInput ).value;
	...
	get inputContentSubmit(): HTMLInputElement {
		...
		inputContentSubmitBtn.addEventListener( 'click', () => {
			let text = ( <HTMLInputElement> this.inputContent ).value as string;
			text.split( '\n' ).forEach( ( text ) => {
				if ( !text.trim() ) {
					return;
				}
				configStorage.addContentToContentList( text.trim() );
			} );
			( <HTMLInputElement> this.inputContent ).value = '';
