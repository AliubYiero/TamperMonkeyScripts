...
// @require		file://D:\Code\tampermonkey-demo\dist\Cangku\CangkuHistoryRecord\CangkuHistoryRecord.js
// ==/UserScript==
...
function transformedPrefSelectorList(prefSelectorList) {
  return Object.entries(
    Object.values(prefSelectorList).flat().reduce((result, current) => ({
      ...result,
      ...current
    }))
  ).map(
...
function parsePost(postList, postCard) {
  const domList = {
    ...
    link: (
      // 主体
      postCard.querySelector(".post-card a") || postCard.querySelector(".sidebar-rank-post-wrap a") || postCard.querySelector(".related-post-card a")
    )
  };
...
async function historyBackup() {
  ...
  if (GMHistoryList.length > historyList.length) {
    ...
    GMHistory.set([.../* @__PURE__ */ new Set([...GMHistoryList, ...historyList])]);
  }
}
