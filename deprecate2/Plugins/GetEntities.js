...
const getEntities = ( srcPath ) => {
	...
	// 获取所有项目的绝对路径(获取`bin.ts`文件的上一层路径)
	...
	for ( const projectPath of projectPathList ) {
		...
		// 获取项目入口
		entity.entryPath = join( projectPath, 'bin.ts' );
...
const getEntries = ( entities ) => {
	const entries = {};
	entities.forEach( entity => {
		const projectName = `${ relative( resolve( 'src' ), dirname( entity.entryPath ) ) }\\${ entity.projectName }`;
		entries[projectName] = entity.entryPath;
	} )
	return entries;
}
...
function userInfoFormat( config, projectName, isProduction = false ) {
	...
	// layui预先配置项
	if ( config.require.includes( 'layui' ) || config.require.includes( 'elementui' ) ) {
		const extraRequire = [];
		config.require = config.require.filter( res => {
			...
			else if ( res === 'elementui' ) {
				extraRequire.push( 'https://cdn.bootcdn.net/ajax/libs/vue/2.6.14/vue.min.js', 'https://cdn.bootcdn.net/ajax/libs/element-ui/2.15.13/index.min.js' );
				config.resource.elementUiCss = 'https://cdn.bootcdn.net/ajax/libs/element-ui/2.15.13/theme-chalk/index.min.css';
				return false;
			}
			return true;
		} )
		config.require = [ ...( new Set( [ ...config.require, ...extraRequire ] ) ) ];
	}
	...
	if ( config.grant[0] ) {
		...
		extraGrantMap.set( 'element', [ 'GM_addElement', 'GM_addStyle' ] );
	...
	const configMap = {
		...
		updateUrl: config.updateUrl || `https://raw.githubusercontent.com/AliubYiero/TamperMonkeyScripts/master/dist/${ projectName }.js`,
		downloadUrl: config.downloadUrl || `https://raw.githubusercontent.com/AliubYiero/TamperMonkeyScripts/master/dist/${ projectName }.js`,
	};
